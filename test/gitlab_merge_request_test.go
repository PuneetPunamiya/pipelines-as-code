...
func TestGitlabMergeRequest(t *testing.T) {
	...
	err = tgitlab.CreateCRD(ctx, projectinfo, runcnx, targetNS, nil, nil)
	assert.NilError(t, err)
...
func TestGitlabDisableCommentsOnMR(t *testing.T) {
	...
	err = tgitlab.CreateCRD(ctx, projectinfo, runcnx, targetNS, nil, settings)
	assert.NilError(t, err)
	...
	notes, _, err := glprovider.Client.Notes.ListMergeRequestNotes(opts.ProjectID, mrID, nil)
	var commentRegexp = regexp.MustCompile(`.*Pipelines as Code CI/*`)
	assert.NilError(t, err)
	successCommentsPost := 0
	for _, n := range notes {
	...
	twait.UpdateRepo(ctx, runcnx.Clients, waitOpts)
